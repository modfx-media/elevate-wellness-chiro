#!/usr/bin/env node
/**
 * Parity check: for each of the 8 core service pages, verify the
 * ServiceTemplate parser retains every paragraph, list item, image, and FAQ
 * pair from the source inventory. Flags anything missing.
 *
 * Run: node scripts/diff-service-pages.mjs
 */
...
// Minimal in-process reimplementation of the parser (parity with lib/parse-service-body.ts).
...
for (const slug of SERVICE_SLUGS) {
  ...
  // Skip leading title-echo blocks the parser deliberately strips.
  ...
  if (parsed.faq.length !== (page.bodyCopy.match(/\n\+/g) || []).length) {
    problems.push(`FAQ count mismatch (${parsed.faq.length} vs source)`);
  }
  ...
  if (missing.length && process.env.VERBOSE) {
    for (const m of missing.slice(0, 5)) {
      console.log(`     · missing: ${m.slice(0, 100)}${m.length > 100 ? "…" : ""}`);
    }
...
console.log("");
console.log(overallOk ? "✅ All 8 service pages parsed cleanly." : "⚠️  Some pages have parity issues (rerun with VERBOSE=1 for details).");
