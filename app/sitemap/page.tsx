              ...
              return (
                <section key={topic.slug} aria-labelledby={`sitemap-${topic.slug}`}>
                  ...
                  <ul className="mt-5 grid gap-x-8 gap-y-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {topicPages.map((page) => (
                      <li key={page.slug}>
                        <Link href={`/${page.slug}/`} className="text-sm font-medium text-white/65 transition-colors hover:text-primary-300">
...
function DirectoryLink({ href, label }: { href: string; label: string }) {
  return (
    <li>
      <Link href={href} className="group inline-flex items-start gap-2 text-sm font-semibold leading-relaxed text-ink-700 transition-colors hover:text-primary-700">
