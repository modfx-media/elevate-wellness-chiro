...
export function PseoLocationTemplate({ page }: { page: PseoPage }) {
  ...
  const content = pseoContentBatch01[page.slug];
  const title = content?.h1 ?? `${page.topic.name} in ${page.city.name}, UT`;
        ...
        <section className="bg-white px-6 py-14 lg:px-8 lg:py-24">
          <div className="mx-auto max-w-[1180px]">
            ...
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-ink-700">
              This page is ready for location-specific content.
            </p>
          </div>
        </section>
      )}
...
function PseoJsonLd({ page }: { page: PseoPage }) {
  ...
  const faqs = pseoContentBatch01[page.slug]?.faqs ?? [];
